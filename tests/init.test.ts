...
describe('Init Command & Project Scaffolding', () => {
  ...
  test('patchReactNativeConfig creates react-native.config.js if none exists', () => {
    ...
    expect(content).toContain("commands: require('jjinppang/commands')");
  });
  ...
  test('updatePackageJson adds dependencies and scripts', () => {
    ...
    expect(pkgJson.devDependencies['jjinppang']).toBe('^0.1.0');
    expect(pkgJson.devDependencies['oxlint']).toBeDefined();
    expect(pkgJson.devDependencies['oxfmt']).toBeDefined();
    expect(pkgJson.scripts['start']).toBe('jjinppang start');
    expect(pkgJson.scripts['start:bun']).toBe('jjinppang start');
    expect(pkgJson.scripts['bundle']).toBe('jjinppang bundle');
    expect(pkgJson.scripts['bundle:ios']).toContain('jjinppang bundle');
    expect(pkgJson.scripts['bundle:android']).toContain('jjinppang bundle');
    expect(pkgJson.scripts['lint']).toBe('jjinppang lint');
    expect(pkgJson.scripts['lint:fix']).toBe('jjinppang lint --fix');
    expect(pkgJson.scripts['format']).toBe('jjinppang format');
    expect(pkgJson.scripts['format:check']).toBe('jjinppang format --check');
    expect(pkgJson.scripts['test']).toBe('jjinppang test');
  ...
  test('updatePackageJson configures macOS and Windows desktop scripts and dependencies', () => {
    ...
    expect(pkgJson.scripts['bundle:macos']).toContain('jjinppang bundle');
    expect(pkgJson.scripts['windows']).toBe('react-native run-windows');
    expect(pkgJson.scripts['bundle:windows']).toContain('jjinppang bundle');
  ...
  test('initExistingProject runs end-to-end configuration successfully', async () => {
    ...
    expect(fs.existsSync(path.join(TEST_DIR, 'jjinppang.config.js'))).toBe(true);
    ...
    expect(updatedPkg.devDependencies['jjinppang']).toBeDefined();
    expect(updatedPkg.scripts['start']).toBe('jjinppang start');
    expect(updatedPkg.scripts['start:bun']).toBe('jjinppang start');
    expect(updatedPkg.scripts['lint']).toBe('jjinppang lint');
    expect(updatedPkg.scripts['format']).toBe('jjinppang format');
    expect(updatedPkg.scripts['test']).toBe('jjinppang test');
    expect(updatedPkg.scripts['bundle']).toBe('jjinppang bundle');
  });
  ...
  test('initExistingProject dryRun does not write any files', async () => {
    ...
    expect(fs.existsSync(path.join(TEST_DIR, 'jjinppang.config.js'))).toBe(false);
