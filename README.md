...
## 📊 5-Way Benchmark Snapshot (Metro vs 찐빵 (jjinppang) vs Rollipop vs Re.Pack vs react-native-esbuild)

Benchmarked on React Native 0.87 with Hermes enabled (`--dev false`, `--reset-cache`, 3-run average). See [BENCHMARK.md](./BENCHMARK.md) for full details and methodology.

| Platform | 번들러 (Bundler) | 핵심 엔진 (Engine) | 전체 빌드 시간 (Avg) | 순수 JS 크기 (Minified JS) | Hermes 바이트코드 (.hbc) | 속도 개선 배수 (vs Metro) |
| :--- | :--- | :--- | :---: | :---: | :---: | :---: |
| **iOS** | **Metro (기본 빌드)** | Babel + Node.js | **9,684 ms** | 1.80 MB | 2.32 MB | 1.0x *(baseline)* |
| **iOS** | **찐빵 (jjinppang - Cold)** | Bun + Babel Hybrid | **4,926 ms** | **1.21 MB** ⚡ | **1.31 MB** ⚡ | **1.97x faster** ⚡ |
| **iOS** | **찐빵 (jjinppang - Warm Cache)** | Bun + Persistent Disk Cache | **2,372 ms** ⚡ | **1.21 MB** ⚡ | **1.31 MB** ⚡ | **4.08x faster** ⚡ *(최고속)* |
| **iOS** | **Rollipop** | Rolldown (Rust) + SWC | **2,849 ms** | 2.23 MB | **1.53 MB** | **3.40x faster** ⚡ |
| **iOS** | **Re.Pack** | Rspack (Rust) + SWC | **9,468 ms** | 1.92 MB | 2.52 MB | **1.02x faster** ⚡ |
| **iOS** | **react-native-esbuild** | esbuild (Go) + Babel | **4,107 ms** | 994.64 KB | 2.05 MB | **2.36x faster** ⚡ |
| **Android** | **Metro (기본 빌드)** | Babel + Node.js | **9,902 ms** | 1.80 MB | 2.33 MB | 1.0x *(baseline)* |
| **Android** | **찐빵 (jjinppang - Cold)** | Bun + Babel Hybrid | **4,682 ms** | **1.21 MB** ⚡ | **1.31 MB** ⚡ | **2.11x faster** ⚡ |
| **Android** | **찐빵 (jjinppang - Warm Cache)** | Bun + Persistent Disk Cache | **2,623 ms** ⚡ | **1.21 MB** ⚡ | **1.31 MB** ⚡ | **3.78x faster** ⚡ *(최고속)* |
| **Android** | **Rollipop** | Rolldown (Rust) + SWC | **2,857 ms** | 2.24 MB | **1.54 MB** | **3.47x faster** ⚡ |
| **Android** | **Re.Pack** | Rspack (Rust) + SWC | **9,401 ms** | 1.93 MB | 2.52 MB | **1.05x faster** ⚡ |
| **Android** | **react-native-esbuild** | esbuild (Go) + Babel | **4,076 ms** | 998.60 KB | 2.06 MB | **2.43x faster** ⚡ |

> [!NOTE]
> **정규화된 번들 크기 및 속도 비교**: 모든 번들러 산출물을 **1) 순수 Minified JS** 및 **2) React Native 공식 Hermes 바이트코드(.hbc)** 로 공정하게 정규화하여 교차 측정한 수치입니다. **`jjinppang`**은 영구 디스크 캐시(`node_modules/.cache/jjinppang`)를 통해 **2.3초대 빌드(4.08x 속도)**를 달성하여 5대 번들러 중 최고속을 기록하며, Hermes 바이트코드(1.31 MB) 역시 **5대 번들러 중 가장 작고 가볍습니다.** 또한 Hermes AOT 컴파일을 번들링 파이프라인에서 일괄 완결합니다.
